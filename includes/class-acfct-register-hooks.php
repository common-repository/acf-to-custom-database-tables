...
class Acf_ct_register_hooks
{
	...
	public function acf_ct_filter_load_value($value, $post_id, $field)
	{
		 ...
		 * Load only in admin
		 */
		// not required as user can load form on frontend
		/*if (is_admin() === false) {
            return $value;
		}*/
	...
	public function acf_ct_filter_pre_update_value($check, $value, $post_id, $field)
	{
		...
		if (
                // if update field support is disabled
			    $is_update_field_support_disabled === true ||

                // if current request is post update request
                (is_admin() === true && $_SERVER['REQUEST_METHOD'] === 'POST' && $_REQUEST['action'] === 'editpost') ||

                // if current request is ACF frontend form update request
				($_SERVER['REQUEST_METHOD'] === 'POST' && $_REQUEST['_acf_screen'] === 'acf_form')
        ) {
			return null;
		}
